...
const Tiket = ({tiket,setTiketadd,setTaskstatus,taskstatus}) => {
  ...
  tiket.status === "In Progress" ? "text-black bg-yellow-200" : "text-black bg-blue-400"}`} 
 >{tiket.status}</p>
    </div>
...
{/* <button onClick={handelcart}>
  <div className="card bg-white rounded-xl p-2 w-full md:w-110 h-[120px] shadow-sm">
    <div className="card-body">
      <div className="flex justify-between">
        <h2 className="card-title font-semibold">{tiket.title}</h2>
        <p
          className={`rounded-lg w-24 h-7 ${
            tiket.status === "Open"
